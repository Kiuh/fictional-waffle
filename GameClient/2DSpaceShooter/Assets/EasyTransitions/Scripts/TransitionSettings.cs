...
{
    [CreateAssetMenu(
        fileName = "TransitionSettings",
        menuName = "Florian Butz/New Transition Settings",
        order = 0
    )]
    public class TransitionSettings : ScriptableObject
    {
        ...
        [HideInInspector]
        public Material addColorMaterial;

        [Header("Transition Settings")]
        [Tooltip(
            "The resolution of the canvas the transition was made in. For some transitions this might change."
        )]
        ...
        [Space(10)]
        [Tooltip(
            "Changes the color tint mode. Multiply just tints the color and Add adds the color to the transition."
        )]
        ...
        [Space(10)]
        [Tooltip(
            "Changes the animation speed of the transition. Only works when theres 1 Animator component somewhere on the transition prefab."
        )]
        [Range(0.5f, 2f)]
        public float transitionSpeed = 1;

        [Tooltip(
            "If you change the transition speed value and set autoAdjustTransitionTime it will automatically change the transition times to fit the new speed."
        )]
